...
const generateCourseJsonFunction = new GameFunction({
  ...
  description:
    'Generate a structured course outline in JSON format based on user goals and duration',
  args: [
  ...
  executable: async (args, logger) => {
    try {
      logger(
        `Generating a ${args.days}-day course outline in JSON format for goal: ${args.userGoal}`,
      );
...
const courseGenWorker = new GameWorker({
  ...
  functions: [generateCourseJsonFunction],
});

async function startGenerateCourse(userGoal: string, days: number) {
  try {
    let resultHash = '';
    if (!process.env.API_KEY) {
      throw new Error('API_KEY is required in environment variables');
    ...
    const courseGenAgent = new GameAgent(process.env.API_KEY, {
      ...
      description:
        'You are an agent that creates structured course outlines based on user goals and duration. You generate content directly in JSON format, and can upload them to IPFS for permanent storage',
